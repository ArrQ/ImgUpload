...
@interface ImgPhotoPickerManager()<UIImagePickerControllerDelegate,UINavigationControllerDelegate,DNImagePickerControllerDelegate>
...
@property(nonatomic,assign) NSInteger maxPhotos;// 选取图片限制
...
@implementation ImgPhotoPickerManager
...
+ (instancetype)shareInstace {
    ...
    dispatch_once (&once, ^{
        sharedInstance = [[ImgPhotoPickerManager alloc] init];
...
- (void)showActionSheetInView:(UIView *)inView fromController:(UIViewController *)fromController andMaxPhoto:(NSInteger)maxPhoto completionBlock:(CompelitionBlock)completionBlock{
    ...
    UIAlertAction *album = [UIAlertAction actionWithTitle:@"从相册选取" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {// 从相册选择
        if ([self isPhotoLibraryAvailable]) {
            ...
            imagePicker.maxPhotos = self.maxPhotos;
