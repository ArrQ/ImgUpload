...
#import "DNAssetsViewCell.h"// 相册图片
#import "DNSendButton.h"
#import "DNAsset.h"
#import "NSURL+DNIMagePickerUrlEqual.h"


static NSUInteger const kDNImageFlowMaxSeletedNumber = 3;// 图片选取限制个数
...
- (instancetype)initWithGroupURL:(NSURL *)assetsGroupURL andPhoto:(NSInteger)photoNum{
...
- (BOOL)seletedAssets:(ALAsset *)asset
{
    ...
    if (self.selectedAssetsArray.count >= self.maxPhotos) {
        
        NSString *constr = [NSString stringWithFormat:@"不能超过%ld张",self.maxPhotos];
        
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"提示" message:constr delegate:self cancelButtonTitle:@"确定" otherButtonTitles:nil, nil];
